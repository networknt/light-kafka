    ...
    for (ConsumerSeekRequest.PartitionOffset partition : request.getOffsets()) {
      ...
      while (true) {
        try {
          ...
          logger.info("seeking to topic = " + partition.getTopic() + " partition = " + partition.getPartition() + " offset = " + partition.getOffset() + " caught illegal state exception will sleep for 1 sec and retry again. Exception is :: " + ie);
          try {
            Thread.sleep(1000);
